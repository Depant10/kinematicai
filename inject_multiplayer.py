#!/usr/bin/env python3
"""Inject multi-player data into the KinematicAI Dashboard HTML."""
import json, gzip, base64, math, shutil, os
import numpy as np
...
for p_key, p_data in players.items():
    ...
    # Need to make sure line has contiguous points for chart to not break, 
    # so we interpolate missing minutes
    last_score = 20
...
# Read from the backup to start fresh
...
# --- Modify the app JS ---
REAL_DATA_JS = f"const REAL_MULTI_DATA = {json.dumps(real_data)};\n"

new_js = app_js

# Add selectedPlayer state
...
# Sparklines
new_js = new_js.replace("const sparkKnee = useMemo(() => genSparkline(101, 1.2), []);", "const sparkKnee = pData.sparkKnee;")
new_js = new_js.replace("const sparkHip = useMemo(() => genSparkline(202, 0.6), []);", "const sparkHip = pData.sparkHip;")
new_js = new_js.replace("const sparkValgus = useMemo(() => genSparkline(303, 1.5), []);", "const sparkValgus = pData.sparkValgus;")

# Stats
new_js = new_js.replace("const matchMinute = 78 + ((tick * 0.04) % 5);", "const matchMinute = REAL_STATS.matchMinute;")
new_js = new_js.replace("const riskIdx = 73 + Math.round(Math.sin(tick / 3) * 1);", "const riskIdx = REAL_STATS.riskIdx;")
new_js = new_js.replace("const confidence = 0.88 + Math.sin(tick / 4) * 0.02 - 0.005;", "const confidence = REAL_STATS.confidence;")
new_js = new_js.replace("const xMax = 90;", "const xMax = REAL_STATS.matchMinute || 90;")
new_js = new_js.replace("const xTicks = [0, 15, 30, 45, 60, 75, 90];", "const xTicks = [0, 15, 30, 45, 60, 75, 90, 105];")

# Player selection UI - replaced "Target Player A"
select_html = """<select value={selectedPlayer} onChange={e => setSelectedPlayer(e.target.value)} style={{background: '#111118', color: '#34d399', border: '1px solid #1e1e2e', padding: '4px', borderRadius: '4px', fontSize: '14px', fontWeight: 'bold', width: '100%', cursor: 'pointer'}}>
                    {Object.keys(REAL_MULTI_DATA).map(k => <option key={k} value={k}>{REAL_MULTI_DATA[k].name}</option>)}
                  </select>"""
new_js = new_js.replace('<div className="player-name">Target Player A</div>', f'<div className="player-name" style={{{{"marginTop":"4px"}}}}> {select_html} </div>')
new_js = new_js.replace('SUBJECT 4711-A', '{pData.team} TEAM')

# Values
new_js = new_js.replace("148.2°", "{REAL_STATS.lKnee}°")
new_js = new_js.replace("141.6°", "{REAL_STATS.rKnee}°")
new_js = new_js.replace("164.8°", "{REAL_STATS.lHip}°")
new_js = new_js.replace("158.1°", "{REAL_STATS.rHip}°")
new_js = new_js.replace("0.31m", "{(REAL_STATS.strideW/100).toFixed(2)}m")

new_js = new_js.replace('value="0.142"', 'value={REAL_STATS.kneeAsym}')
new_js = new_js.replace('value="6.8"', 'value={Math.abs(REAL_STATS.lHip - REAL_STATS.rHip).toFixed(1)}')
new_js = new_js.replace('value="2.41"', 'value={REAL_STATS.valgusAsym}')
new_js = new_js.replace('trend={18.4}', 'trend={REAL_STATS.kneeChange}')
new_js = new_js.replace('trend={12.4}', 'trend={REAL_STATS.hipChange}')

new_js = new_js.replace("DIVERGENCE ONSET · 55'", "DIVERGENCE ONSET · {REAL_STATS.divergeMin}'")
new_js = new_js.replace("{sx(55)}", "{sx(REAL_STATS.divergeMin)}")

new_js = new_js.replace("117,432", "30,989")
new_js = new_js.replace("MOTIONBERT-L · 198M", "YOLO11m-pose · Apple MPS")
new_js = new_js.replace("FP16 · TENSORRT", "YOLO11m · MPS")
new_js = new_js.replace("FEED <b>SKY-01</b>", "DATA <b>{REAL_STATS.dataPoints} pts</b>")
new_js = new_js.replace("WINDOW · 90:00 LIVE", "WINDOW · {REAL_STATS.matchMinute}:00 ANALYZED")

# Stats fields
new_js = new_js.replace("10.84 km", "{REAL_STATS.matchMinute} min")
new_js = new_js.replace(">DISTANCE<", ">DURATION<")
new_js = new_js.replace(">31<", ">{REAL_STATS.dataPoints}<")
new_js = new_js.replace(">HI SPRINTS<", ">DATA POINTS<")
new_js = new_js.replace("1.42", "{REAL_STATS.kneeChange}%")
new_js = new_js.replace(">ACWR (28d)<", ">KNEE Δ<")
new_js = new_js.replace("L HAM 38d", "HIP {REAL_STATS.hipChange > 0 ? '+' : ''}{REAL_STATS.hipChange}%")
...
print("✅ Multi-player Dashboard generated!")

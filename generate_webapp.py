...
# Convert App JS into React component source
# Remove ReactDOM render at bottom
app_js = app_js.replace("ReactDOM.createRoot(document.getElementById('root')).render(<App />);", "export default App;")

# Fix the import
app_js = app_js.replace("const { useState, useEffect, useMemo, useRef } = React;", "import { useState, useEffect, useMemo, useRef } from 'react';\nimport { REAL_MULTI_DATA } from './data';\nimport './index.css';")

# Make App component dynamic
...
# Pass max directly to TemporalRiskChart since it is a prop
app_js = app_js.replace("const xMax = 90;", "const xMax = xMaxProp || 90;")
app_js = app_js.replace("function TemporalRiskChart({ matchMinute, baseline, live }) {", "function TemporalRiskChart({ matchMinute, baseline, live, xMaxProp }) {")
app_js = app_js.replace("<TemporalRiskChart matchMinute={matchMinute} baseline={baseline} live={live}/>", "<TemporalRiskChart matchMinute={matchMinute} baseline={baseline} live={live} xMaxProp={REAL_STATS.matchMinute}/>")
...
print("Writing CSS...")
# Extract all the fonts and other CSS. We just copy the entire block from the backup HTML.
css_start = html.find('<style>') + 7
css_end = html.find('</style>', css_start)
css_content = html[css_start:css_end]
with open('dashboard-app/src/index.css', 'w') as f:
    f.write(css_content)

print("Done generating React codebase!")
